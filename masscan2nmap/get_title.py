...
def worker():
    ...
    try:
        ...
        try:
            contentDecode = r.content.decode('utf-8')
        except:
            try:
                contentDecode = r.content.decode('gbk')
            except:
                print('[+] html decode error!url:{}'.format(url))
        regex = re.compile(r'<title>(.*)</title>')
        result = regex.search(contentDecode)
        # 指定宽度格式化输出
        message = f'{url.ljust(40)}  {str(status).ljust(10)}  {result.group(1).ljust(40)}'
        print(message)
        sem.acquire()
...
def get_title():
    with open('url.txt', 'r') as f9:
        lines = f9.readlines()
        for line in lines:
            url = line.strip('\n')
            tasks.put(url)

    all_requests = [gevent.spawn(boss) for i in range(30)]
    gevent.joinall(all_requests)
    with open('url_title.txt', 'a') as f3:
        f3.write('\n' + f'-------------------------------------------  {dt} -------------------------------------------' + '\n' + '\n')
